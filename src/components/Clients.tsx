  ...
  return (
    <section id="clients" className="py-16 md:py-24 bg-gray-50/50 relative overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Desktop & Mobile Grid - Cards Style (RV Tec Reference) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 max-w-5xl mx-auto">
          {clients.map((client, index) => (
            <div
              key={index}
              className="group bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 p-6 md:p-8 flex items-center justify-center h-24 md:h-32 transition-shadow duration-300"
            >
              <img
                src={client.logo}
                alt={`Logo ${client.name}`}
                className="h-10 md:h-12 w-auto max-w-full object-contain grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300"
                loading="lazy"
                width={120}
                height={48}
                decoding="async"
