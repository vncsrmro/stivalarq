...
function Film({ slot, paused, className = '', onReady }: { slot: MediaSlot; paused: boolean; className?: string; onReady?: () => void }) {
  ...
  return <div ref={host} className={`film ${className}`}>
    ...
    {near && slot.video && !failed && <video ref={video} className={playing ? 'is-playing' : ''} muted loop playsInline preload={paused ? 'none' : 'auto'} autoPlay={!paused && visible} poster={slot.poster} onLoadedData={onReady} onPlaying={() => { setPlaying(true); onReady?.(); }} onError={e => { if (e.target === e.currentTarget) { setFailed(true); onReady?.(); } }} aria-hidden="true">
  ...
  return <div ref={root} className={`stival-site ${ready ? 'is-ready' : ''} ${reduced ? 'reduce-motion' : ''}`}>
    ...
    <main id="conteudo">
      <section className="hero" id="inicio" aria-labelledby="hero-title">
        ...
        <div className="hero-topline"><span>ARQUITETURA & CONSULTORIA EM SAÚDE</span><span>GUSTAVO STIVAL · BRASIL</span></div>
    ...
    <footer className="site-footer" aria-label="Rodapé">
      <div className="footer-grid">
        <div className="footer-brand">
          ...
          <p>Arquitetura e consultoria em saúde.</p>
